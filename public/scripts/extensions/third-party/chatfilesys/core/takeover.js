 ...
 *
 * 问题：宿主点原生按钮时会**先落一个复制文件**（`chats/save` 到一个新键），分支只存在于
 * 那个文件里。纯库模式下不要文件、要库内分支，所以必须在写路径的「未命中家族」分支里
 * 认出「这次写的是某个已是家族聊天的新分支/检查点」，并把落点改成库内一条新分支。
 *
 ...
 *  2. 正文（去 header 后的行序列）必须是**父分支当前投影的逐行前缀**（保守闸门：宁可透传不误吞）
...
/** 分支 path 的楼层数（path 的键是 1..N 连续楼层号） */
 ...
 * `active_branch` 解析的，若把它切到新分支，用户点「返回父聊天」回到父键时会被投影到
 * 新分支（只剩截断前缀）——内容就错了。改为「键 → 分支」显式绑定后：
 *   父键仍按 `active_branch` 解析（接管不动它 → 内容 = 完整历史）；
 *   新键按自己的绑定解析（内容 = 到分叉点为止）。
 * 父键**不**建绑定：`active_branch` 是 UI「切换分支」的作用面，父键跟随它才是用户预期；
 ...
 * @param {Array<string>} args.parentContents 父分支投影行的 content 字符串
 * @param {object} args.parentModel 父家族模型（store-bridge 形态）
 * @param {string} args.parentBranchId 分叉来源分支（父键当前所在的分支）
 * @param {string} args.parentKey 父键（仅用于日志/诊断）
 * @param {string} args.newKey 新键
 * @param {string} args.fileName 新键文件名（分支名用）
...
export function planTakeover({
    ...
    // 分叉来源 = 父键当前所在的分支（父键可能只是家族里的一个分支键，不一定是 active_branch）
...
/**
 * 按聊天键解析「这次读写作用于哪条分支」。
 * 键绑定优先（原生分支/检查点键各自代表一条分支），无绑定回落到家族活跃分支
 * （导入建档的主键、以及 UI 在某个键上切换分支后由该键绑定跟随的情形）。
